...
def verify_receipt(receipt: dict, pubkey_hex: str | None) -> tuple[bool, str]:
    """Return (ok, detail). ok requires integrity, plus signature if a key given."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    pubkey_hex = _load_pubkey_hex(args)
    ...
    for i, receipt in enumerate(receipts):
        ok, detail = verify_receipt(receipt, pubkey_hex)
        flag = "OK " if ok else "FAIL"
        print(f"  [{flag}] receipt {i + 1}/{len(receipts)} "
              f"({receipt.get('spec')}): {detail}")
        n_ok += int(ok)
    print(f"\n{n_ok}/{len(receipts)} receipts verified"
          f"{' (with authenticity)' if pubkey_hex else ' (integrity only)'}.")
